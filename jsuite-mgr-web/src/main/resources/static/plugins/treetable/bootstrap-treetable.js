(function ($) {
    ...
    // 初始化
    BootstrapTreeTable.prototype.init = function (params) {
        // 初始化配置
        this.initOptions();
        // 初始化容器
        this.initContainer();
    ...
    BootstrapTreeTable.prototype.initToolbar = function () {
        ...
        if (self.options.showSearch) {
            ...
            self.registerSearchBtnClickEvent($searchBtn);
        }
        // 是否显示刷新按钮
        if (self.options.showRefresh) {
            var $refreshBtn = $('<button class="btn btn-default btn-outline refresh_btn" type="button" aria-label="refresh" title="刷新">' +
        ...
        // 是否显示列选项
        if (self.options.showColumns) {
            var $columns_div = $('<div class="btn-group pull-right" title="列">' +
                ...
                '<i class="glyphicon glyphicon-list"></i> <span class="caret"></span></button></div>');
    ...
    BootstrapTreeTable.prototype.initHeader = function () {
        ...
        $.each(_columns, function (i, column) {
            ...
            if (i == 0 && (column.checkbox || column.radio)) {
                ...
                $th = $('<th  class="' + column.field + '_cls"></th>');
                if (column.width) {
    ...
    // 初始化数据服务
    BootstrapTreeTable.prototype.initServer = function (params) {
        var self = this;
        // 加载数据前先清空
        self.data_list = {};
        self.data_obj = {};
        // var $tbody = self.$el.find("tbody");
        // self.showEmpty("加载中,请稍后.....");
        if (self.options.url) {
            $.ajax({
                ...
                success: function (data, textStatus, jqXHR) {
                    if (data.status == 200) {
                        ...
                        this.error(jqXHR, data.status, data.message);
                    }
                },
                error: function (res, textStatus, message) {
                    if (message) {
                        self.showEmpty(message);
                    } else {
                        self.showEmpty(res.responseText);
                    }
                    self.autoReSize();
    ...
    BootstrapTreeTable.prototype.showEmpty = function (message) {
        var self = this;
        var $tbody = self.$el.find("tbody");
        var _empty = '<tr><td colspan="' + self.options.columns.length + '"><div style="display: block;text-align: center;">' + message + '</div></td></tr>'
        $tbody.html(_empty);
        self.autoReSize();
    };
    // 加载完数据后渲染表格
    ...
    // 检索信息按钮点击事件
    BootstrapTreeTable.prototype.registerSearchBtnClickEvent = function (btn) {
    ...
    // 刷新数据
    BootstrapTreeTable.prototype.getSelection = function (id) {
        var self = this;
        if (self.data_obj == null) {
            return null;
        }
        var value = self.data_obj["id_" + id];
        if (value == undefined) {
            return null;
        }
        return value;
